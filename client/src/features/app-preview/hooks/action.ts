import { useMutation, useQueryClient } from 'react-query';

import { useAtom, useAtomValue } from 'jotai';
import { WIDGET_PREVIEW_QUERY_KEY } from '@/features/app-preview/hooks';
import { workerAxios } from '@/lib/axios';
import { fetchJobStatus } from '@/utils/worker-job';
import { pageAtom } from '@/features/page';
import { pageStateContextAtom, useSyncState } from '@/features/app-state';
import { getErrorMessage } from '@/utils';
import { useToast } from '@/lib/chakra-ui';

export const executeAction = async ({ pageName, appName, pageState }: any) => {
	const response = await workerAxios.post(`/function/class`, {
		// @Yash update me
		page_name: pageName,
		app_name: appName,
		// TODO: action should be a composition of 'on_click_' and button name
		action: 'on_click_button1',
		// TODO: target should be the parents name, could be widget or table
		target: 'widget1',
		state: pageState.state,
	});

	if (response.data?.job_id) {
		const jobResponse = await fetchJobStatus(response.data.job_id);
		return jobResponse;
	}
	throw new Error('Failed to run python function');
};

export const useExecuteAction = (props: any = {}) => {
	const queryClient = useQueryClient();

	return useMutation(executeAction, {
		...props,
		onSettled: () => {
			queryClient.invalidateQueries(WIDGET_PREVIEW_QUERY_KEY);
		},
	});
};

export const useEvent = (props: any) => {
	const toast = useToast();
	const [{ pageName, appName }] = useAtom(pageAtom);
	const pageStateContext = useAtomValue(pageStateContextAtom);

	const syncState = useSyncState();

	const actionMutation = useExecuteAction({
		onSuccess: (data: any) => {
			syncState(data);
			props?.onSuccess?.(data);
		},
		onError: (error: any) => {
			toast({
				status: 'error',
				title: 'Failed to execute action',
				description: getErrorMessage(error),
			});
			props?.onError?.();
		},
	});

	const handleAction = () => {
		actionMutation.mutate({
			pageName,
			appName,
			pageState: pageStateContext,
		});
	};

	const handleEvent = () => {
		handleAction();
	};

	return { handleEvent, mutation: actionMutation };
};
