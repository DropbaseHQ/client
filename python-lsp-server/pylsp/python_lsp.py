    ...
    with ThreadPoolExecutor(max_workers=10) as tpool:
        ...
        async def pylsp_ws(websocket):
            ...
            pylsp_handler = handler_class(
                rx=None, tx=None, consumer=response_handler, check_parent_process=check_parent_process
            )
...
class PythonLSPServer(MethodDispatcher):
    ...
    def capabilities(self):
        server_capabilities = {
            ...
            "executeCommandProvider": {"commands": flatten(self._hook("pylsp_commands"))},
            "hoverProvider": True,
    ...
    def m_initialize(
        ...
        **_kwargs
        ...
        self.config = config.Config(
            rootUri, initializationOptions or {}, processId, _kwargs.get("capabilities", {})
        )
        ...
        if workspaceFolders:
            for folder in workspaceFolders:
                ...
                workspace_config = config.Config(
                    uri, self.config._init_opts, self.config._process_id, self.config._capabilities
                )
    ...
    def references(self, doc_uri, position, exclude_declaration):
        return flatten(
            self._hook(
                "pylsp_references", doc_uri, position=position, exclude_declaration=exclude_declaration
            )
        )
    ...
    def m_notebook_document__did_open(self, notebookDocument=None, cellTextDocuments=None, **_kwargs):
        ...
        for cell in cellTextDocuments or []:
            workspace.put_cell_document(
                cell["uri"], cell["languageId"], cell["text"], version=cell.get("version")
            )
            ...
            if structure:
                ...
                if cell_delete_count == 0:
                    ...
                    workspace.add_notebook_cells(
                        notebookDocument["uri"], notebook_cell_array_change["cells"], start
                    )
                else:
                    # Case 3
                    # Cell documents
    ...
    def m_workspace__did_change_workspace_folders(
        ...
        for added_info in added:
            if "uri" in added_info:
                ...
                workspace_config = config.Config(
                    added_uri, self.config._init_opts, self.config._process_id, self.config._capabilities
                )
                workspace_config.update(self.config._settings)
