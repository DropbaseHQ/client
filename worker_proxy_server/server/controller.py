...
class _FRPClientManager:
    ...
    def handle_client_ping(self, token: str):
        ...
        if client:
            ...
            self.logger.warning(f"Ping failed: client \"{token}\" does not exist.")
    ...
    def get_tunnel(self, token: str, type: TunnelType) -> tuple[str, int] | None:
        try:
            ...
            self.logger.warning(f"Failed to get tunnel \"{type}\" from client \"{token}\".")
    ...
    def remove_tunnel(self, token: str, *, type: TunnelType):
        try:
            ...
            self.logger.warning(f"Failed to remove tunnel \"{type}\" from client \"{token}\": not found.")
