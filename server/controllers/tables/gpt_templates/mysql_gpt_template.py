def get_mysql_gpt_input(db_schema: dict, user_sql: str, column_names: list) -> str:
...
```
This is only a sample value, only name the database_name "public" if it is actually public. The database name is the value right after "database"
...
"database" is the database, this can be found by referencing the Database schema.
If not otherwise specified, use the default database specified in the Database schema metadata.
