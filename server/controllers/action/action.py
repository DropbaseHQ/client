...
def get_app_schema() -> ActionSchema:
...
def get_db_schema(engine) -> ActionSchema:
    inspector = inspect(engine)
    schemas = inspector.get_schema_names()
    default_search_path = inspector.default_schema_name

    database_structure: ActionSchema = {
        "metadata": {
...
def get_app_details(db: Session, app_id: str):
    ...
    app_functions = crud.functions.get_action_functions(db, app_id=app_id)
    app_components = crud.components.get_action_component(db, app_id=app_id)
    organized_functions = {
        "fetchers": [],
