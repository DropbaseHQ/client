...
def test_run_task():
    ...
    components = crud.components.get_app_component(db, app_id=ValueStorage.app_id)
    components.dataclass = user_input_str

    # get table schema for row
    sqls = crud.sqls.get_app_sqls(db, app_id=ValueStorage.app_id)
    sqls[0].dataclass = row_str

    # get function code
    functions = crud.functions.get_app_functions(db, app_id=ValueStorage.app_id)
